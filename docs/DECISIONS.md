...
- 2026-09-24 Chat apps: added `install/short.txt` (≤1,500 characters, checked in CI) for ChatGPT / Gemini Gems / Grok custom instructions. Limits from secondary sources, September 2026: ChatGPT 1,500 free / 5,000 paid; Grok 4,000; Gemini Gems no practical limit (Google suggests 500–2,000). Menu names: ChatGPT "Settings → Personalization → Custom instructions", Gemini "Explore Gems → New Gem" (Google help), Grok "Settings → Customize". Not tested in those apps; a Claude-as-chat-app proxy test with only short.txt gave one-question, glossed replies in 3 of 4 prompts (the miss was the known "asks two things" on permission denied).
- 2026-09-24 Gemini CLI: added `install/GEMINI.md.snippet` (SKILL.md body verbatim, synced by check.py).
- 2026-09-24 Chat apps: README shows the short text inline in a copyable code box (user request: "just output the text to paste"). Added `install/short.ja.txt` (739 characters) for the Japanese README; proxy test gave glossed, one-question, one-next-action replies in 3/3 prompts.
